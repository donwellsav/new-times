...
export class FeedbackDetector {
      ...
      if (valid) {
        ...
        if (hold[i] >= this.config.sustainMs && active[i] === 0) {
          ...
          ;(peak as DetectedPeak & { 
            ...
            bandwidthHz: number
            msd: number
            msdGrowthRate: number
            msdIsHowl: boolean
            msdFastConfirm: boolean
          }).msd = msdResult.msd
          ;(peak as DetectedPeak & { 
            qEstimate: number
            bandwidthHz: number
            ...
            msdIsHowl: boolean
            msdFastConfirm: boolean
          }).msdGrowthRate = msdResult.growthRate
          ;(peak as DetectedPeak & { 
            qEstimate: number
            bandwidthHz: number
            msd: number
          ...
          ;(peak as DetectedPeak & { 
            ...
            msdIsHowl: boolean
            msdFastConfirm: boolean
          }).msdFastConfirm = msdResult.fastConfirm
          ...
          }).persistenceFrames = persistenceResult.frames
          ;(peak as DetectedPeak & {
            persistenceFrames: number
            persistenceBoost: number
            isPersistent: boolean
            isHighlyPersistent: boolean
          }).persistenceBoost = persistenceResult.boost
          ;(peak as DetectedPeak & {
            persistenceFrames: number
            persistenceBoost: number
            isPersistent: boolean
            isHighlyPersistent: boolean
          }).isPersistent = persistenceResult.isPersistent
          ;(peak as DetectedPeak & {
            persistenceFrames: number
            persistenceBoost: number
            isPersistent: boolean
            isHighlyPersistent: boolean
          }).isHighlyPersistent = persistenceResult.isHighlyPersistent
  ...
  private calculateMsd(binIndex: number): { msd: number; growthRate: number; isHowl: boolean; fastConfirm: boolean } {
    ...
    for (let i = 0; i < frameCount; i++) {
      const idx = (currentIdx - frameCount + i + historySize) % historySize
      ordered.push(history[idx])
    }
    
    // Calculate first derivative (gradient between consecutive frames)
    const firstDerivative: number[] = []
    for (let i = 1; i < ordered.length; i++) {
      firstDerivative.push(ordered[i] - ordered[i - 1])
    }
    
    // Calculate average growth rate (dB per frame)
    const avgGrowthRate = firstDerivative.length > 0 
      ? firstDerivative.reduce((a, b) => a + b, 0) / firstDerivative.length
      : 0
    ...
    // Summing MSD: sum of squared second derivatives
    // Lower value = more consistent growth = more likely feedback
    let sumSquaredSecondDeriv = 0
    for (const d2 of secondDerivative) {
      sumSquaredSecondDeriv += d2 * d2
    }
    
    // Normalize by number of samples
    const msd = secondDerivative.length > 0 
      ? Math.sqrt(sumSquaredSecondDeriv / secondDerivative.length)
      : 0

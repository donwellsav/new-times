...
const SIDEBAR_DEFAULT = 320
  const [sidebarWidth, setSidebarWidth] = useState<number>(() => {
    ...
    return stored ? Math.min(SIDEBAR_MAX, Math.max(SIDEBAR_MIN, Number(stored))) : SIDEBAR_DEFAULT
  })
  ...
  useEffect(() => {
    ...
    const onMouseUp = () => {
      ...
      setSidebarWidth(prev => {
        localStorage.setItem('ktr-sidebar-width', String(prev))
        return prev
      })
    }
    ...
    <div className="flex flex-col h-screen">
      ...
      <header className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-border bg-card/80 backdrop-blur-sm sm:px-4 sm:py-2 sm:gap-4">
        ...
        <div className="flex items-stretch justify-end pr-12 px-2 gap-2 h-10 sm:h-auto sm:justify-start sm:pr-0 sm:px-0 sm:gap-3 sm:flex-shrink-0">
          ...
          <div className="hidden sm:flex items-start gap-2.5 flex-shrink-0">
            <div className="flex flex-col items-center gap-0">
              <button
                onClick={isRunning ? stop : start}
                aria-label={isRunning ? 'Stop analysis' : 'Start analysis'}
                className="relative w-10 h-10 flex items-center justify-center flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-full"
              >
              ...
              {!isRunning && (
                <span 
                  onClick={start}
                  className="text-[10px] font-black whitespace-nowrap text-primary/70 hover:text-primary cursor-pointer transition-colors -mt-2"
                >
                  START
                </span>
              )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {!mobileShowGraph && (
          <div className="landscape:hidden flex-1 flex flex-col overflow-hidden bg-background">
            ...
            <div className="border-b border-border p-3 flex-shrink-0 bg-card/50 overflow-y-auto max-h-64">
              <DetectionControls settings={settings} onModeChange={handleModeChange} onSettingsChange={handleSettingsChange} />
            </div>
        ...
        <aside
          className="hidden landscape:flex flex-shrink-0 border-r border-border bg-card/50 flex-col overflow-hidden relative"
        ...
        >
          ...
          >
            {/* Visible hit-area highlight on hover */}
            <div className="h-full w-full bg-transparent group-hover:bg-primary/40 transition-colors duration-150" />
          </div>
        </aside>
